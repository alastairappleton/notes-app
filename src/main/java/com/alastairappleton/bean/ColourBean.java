...
public class ColourBean implements Serializable {
  ...
  public String add() {
    ...
    if (this.colour.getColourCode().isEmpty() | this.colour.getColourName().isEmpty()) {
      return "settings?faces-redirect=true"; // Redirect so we do not resubmit data when hitting 'F5' to refresh;
    }
  ...
  public String update(Colour colour) {
    ...
    return "settings?faces-redirect=true"; // Redirect so we keep data when refreshing
